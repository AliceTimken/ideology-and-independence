...
#load('~/Desktop/Phd Program/Fall 2024/Spring 2025/Quant Model Submission/ideology_independence_data_3_2026.RData')

load('~/Desktop/Phd Program/Fall 2024/Spring 2025/Quant Model Submission/ideology_independence_data_3_11_2026.RData')
...
df.poland.only <- subset(df.model, country_name == "Poland")
...
### Range of LJI across regions (within model data)
...
ggplot(data = df.model, mapping = aes(x = api_gov, y = popul_gov)) +
  geom_point(alpha = ifelse(grepl("POL", df.model$cy), 1, 0.8), color = ifelse(grepl("POL", df.model$cy), "red", "cyan4"), shape = ifelse(grepl("POL", df.model$cy), 17, 16)) +
  theme_bw() +
  ylim(0,1) +
  xlim(0,1) +
  geom_text_repel(size= 3.3,
                  aes(label = ifelse(grepl("POL", df.model$cy), cy, "")),
...
ggplot(data = df.parties, mapping = aes(x = api_gov, y = popul_gov)) +
  geom_point(alpha = ifelse(grepl("Poland", df.parties$cy2), 1, 0.5), color = ifelse(grepl("Poland", df.parties$cy2), "red", "cyan4"), shape = ifelse(grepl("Poland", df.parties$cy2), 17, 16)) +
  theme_bw() +
  ylim(0,1) +
  xlim(0,1) +
...
# API plot models
plotmodels <- list(
  "Legislative"  = lm_robust(LJI_lag1 ~ api_gov + popul_gov,
                                data = df.model),
  "Legislative F.E."  = lm_robust(LJI_lag1 ~ api_gov+popul_gov + conserv_gov +polconiii + frac +  log_e_gdppc+ boix_democracy_duration_log + as.factor(country_name) + allhouse,
                                data = df.model, cluster = country_name),
  "Executive" = lm_robust(LJI_lag1 ~ api_exec_party + popul_exec_party,
                                data = df.model),
  "Executive F.E."  = lm_robust(LJI_lag1 ~ api_exec_party + popul_exec_party + conserv_exec_party + polconiii + frac +  log_e_gdppc+ boix_democracy_duration_log + as.factor(country_name) + allhouse,
                                data = df.model, cluster = country_name))
...
modelplot(plotmodels, size = 0.6, coef_omit = "as.factor|Intercept", coef_map = cm_plot)+
  aes(shape = model) +
  theme_bw() +
  scale_color_manual(values = c("gray70", "black", "gray70", "black"))+
  b

```

## Casing the Polish Case

```{r residual and other plots main models}

library(broom)

# X & Y

# Plot competition vs. lji
ggplot(df.model, aes(x = polconiii, y = LJI_lag1)) +
  geom_point(alpha = ifelse(grepl("Poland", df.model$country_name), 1, 0.3), color = ifelse(grepl("Poland", df.model$country_name), "red", "black")) +
  geom_hline(yintercept = mean(df.model$LJI_lag1), linetype = "dashed", color = "darkred") +
  geom_vline(xintercept = mean(df.model$polconiii), linetype = "dashed", color = "darkred") +
  labs(title = "Legislative FE Model Residual Plot",
       x = "Competition",
       y = "Judicial Independence (LJI)") +
  theme_minimal() +
  geom_smooth(method = "loess") + 
  geom_text_repel(size= 3.3,
                  aes(label = ifelse(grepl("Poland", country_name), as.character(cy2), "")),
                  min.segment.length = 0,
                  point.padding = 0)

# Plot competition vs. lji
ggplot(df.model, aes(x = api_gov, y = LJI_lag1)) +
  geom_point(alpha = ifelse(grepl("Poland", df.model$country_name), 1, 0.3), color = ifelse(grepl("Poland", df.model$country_name), "red", "black")) +
  geom_hline(yintercept = mean(df.model$LJI_lag1), linetype = "dashed", color = "darkred") +
  geom_vline(xintercept = mean(df.model$api_gov), linetype = "dashed", color = "darkred") +
  labs(title = "Legislative FE Model Residual Plot",
       x = "Anti-Pluralism (Legislative)",
       y = "Judicial Independence (LJI)") +
  theme_minimal() +
  geom_smooth(method = "loess") + 
  geom_text_repel(size= 3.3,
...
# Plot competition vs. lji
ggplot(df.model, aes(x = popul_gov, y = LJI_lag1)) +
  geom_point(alpha = ifelse(grepl("Poland", df.model$country_name), 1, 0.3), color = ifelse(grepl("Poland", df.model$country_name), "red", "black")) +
  geom_hline(yintercept = mean(df.model$LJI_lag1), linetype = "dashed", color = "darkred") +
  geom_vline(xintercept = mean(df.model$popul_gov), linetype = "dashed", color = "darkred") +
  labs(title = "Legislative FE Model Residual Plot",
       x = "Anti-Elitism (Legislative)",
       y = "Judicial Independence (LJI)") +
  theme_minimal() +
  geom_smooth(method = "loess") + 
  geom_text_repel(size= 3.3,
...
# fe legislative
legislative.lm = lm(LJI_lag1 ~ api_gov+popul_gov + conserv_gov +polconiii + frac +  log_e_gdppc+ boix_democracy_duration_log + as.factor(country_name) + allhouse,
                                data = df.model)
legislative.res = resid(legislative.lm)
# Augment the data with residuals and fitted values
augmented_legislative.lm <- augment(legislative.lm)

# Rename country_name column
colnames(augmented_legislative.lm)[colnames(augmented_legislative.lm) == "as.factor(country_name)"] <- "country_name"

# Plot residuals vs. fitted values
ggplot(augmented_legislative.lm, aes(x = .fitted, y = LJI_lag1)) +
  geom_point(alpha = ifelse(grepl("Poland", augmented_legislative.lm$country_name), 1, 0.3), color = ifelse(grepl("Poland", augmented_legislative.lm$country_name), "red", "black")) +
  #geom_hline(yintercept = 0, linetype = "dashed", color = "darkred") +
  labs(title = "Legislative FE Model vs. Real Y Values",
       x = "Fitted Model Values (Predicted Y)",
       y = "Judicial Independence (LJI)") +
  theme_minimal() +
  geom_smooth(method = "loess") + 
  geom_text_repel(size= 3.3,
                  aes(label = ifelse(grepl("Poland", country_name), as.character(country_name), "")),
                  min.segment.length = 0,
                  point.padding = 0)


# fe executive
executive.lm = lm(LJI_lag1 ~ api_exec_party + popul_exec_party + conserv_exec_party +polconiii + frac +  log_e_gdppc+ boix_democracy_duration_log + as.factor(country_name) + allhouse,
                                data = df.model)
executive.res = resid(executive.lm)
# Augment the data with residuals and fitted values
augmented_executive.lm <- augment(executive.lm)

# Rename country_name column
colnames(augmented_executive.lm)[colnames(augmented_executive.lm) == "as.factor(country_name)"] <- "country_name"

# Plot residuals vs. fitted values
ggplot(augmented_executive.lm, aes(x = .fitted, y = LJI_lag1)) +
  geom_point(alpha = ifelse(grepl("Poland", augmented_executive.lm$country_name), 1, 0.3), color = ifelse(grepl("Poland", augmented_executive.lm$country_name), "red", "black")) +
  #geom_hline(yintercept = 0, linetype = "dashed", color = "darkred") +
  labs(title = "Executive FE Model vs. Real Y Values",
       x = "Fitted Model Values (Predicted Y)",
       y = "Judicial Independence (LJI)") +
  theme_minimal() +
  geom_smooth(method = "loess") + 
